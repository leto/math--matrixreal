...
metafile : create_distdir
	...
	$(NOECHO) $(ECHO) 'name:         Math-MatrixReal' >> META_new.yml
	$(NOECHO) $(ECHO) 'version:      2.02' >> META_new.yml
	$(NOECHO) $(ECHO) 'version_from: MatrixReal.pm' >> META_new.yml
...
ppd:
	$(NOECHO) $(ECHO) '<SOFTPKG NAME="$(DISTNAME)" VERSION="2,02,0,0">' > $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <TITLE>$(DISTNAME)</TITLE>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <ABSTRACT>Manipulate Matrices of Real Numbers</ABSTRACT>' >> $(DISTNAME).ppd
	$(NOECHO) $(ECHO) '    <AUTHOR>Jonathan Leto &lt;jonathan@leto.net&gt;</AUTHOR>' >> $(DISTNAME).ppd
...
pm_to_blib : $(TO_INST_PM)
	$(NOECHO) $(ABSPERLRUN) -MExtUtils::Install -e 'pm_to_blib({@ARGV}, '\''$(INST_LIB)/auto'\'', '\''$(PM_FILTER)'\'')' \
	  Kleene.pod $(INST_LIB)/Math/Kleene.pod \
	  MatrixReal.pm $(INST_LIB)/Math/MatrixReal.pm \
	  funcs.pl $(INST_LIB)/Math/funcs.pl 
